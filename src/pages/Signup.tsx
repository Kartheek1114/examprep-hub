...
import { BookOpen, Mail, Lock, Eye, EyeOff, User, Check, X } from "lucide-react";
...
const Signup = () => {
  ...
  const ValidationRequirement = ({ met, label }: { met: boolean; label: string }) => (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPasswordValid) {
      toast.error("Password must be > 8 characters, contain a number and a special character");
      return;
    }
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex flex-1 gradient-hero items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative text-center max-w-md">
          ...
          <h2 className="font-heading text-3xl font-bold text-primary-foreground mb-4">Join Prepnovus Today</h2>
          <p className="text-primary-foreground/70">Start your preparation with 1 Lakh+ questions, detailed analytics, and timed practice sessions.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <p className="text-muted-foreground mb-8">Start your exam preparation journey</p>

          <div className="flex justify-center mb-6">
            <button
              onClick={handleGoogleLogin}
              className="flex items-center justify-center w-full h-11 rounded-lg border border-border bg-card hover:bg-muted transition-colors hover-lift gap-2 font-medium"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full gradient-primary text-primary-foreground border-0 shadow-primary-glow hover:opacity-90 transition-opacity">
              Create Account
            </Button>
          </form>
